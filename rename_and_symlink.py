...
if __name__ == "__main__":
    while True:
        ...
        log(f"[INFO] Sleeping for {POLL_INTERVAL} seconds before next scan.")
        time.sleep(POLL_INTERVAL)
, '', title)  # Remove year and everything after
        title = title.strip()
        
        # Skip if the cleaned title is empty or matches the source folder name
        if not title or title.lower() == SOURCE.name.lower():
